"""
Collect task: stack_two_cubes.

Same flow as `collect_cup_bowl.py` -- pick object, place on target,
reset for next round -- but the target is another cube (not a container or
flat mat). Placement lands cube_a on top of cube_b via moveUntilContact
(contact height = cube_b's top face).

Per episode:
  start:  cube_a + cube_b on table at 2 random separated positions
  TASK (recorded): pick cube_a -> stack on top of cube_b (moveUntilContact)
  RESET (NOT recorded): pick cube_a off cube_b -> place at cube_a_new
                        pick cube_b -> place at cube_b_new
                        return home

Motion parameters come from the `verify:` block of a plan yaml (default
`collect/plan_stack_two_cubes.yaml`), shared with `verify_two_cubes.py`
so verify + collect always agree on numbers.

Usage:
    /home/joshua/Desktop/ur5e_vla/.venv/bin/python -u collect_two_cubes.py -n 50
"""
import argparse
import json
import os
...
TASK_NAME = "stack_two_cubes"
DEFAULT_PLAN = os.path.join(THIS_DIR, "collect", "plan_stack_two_cubes.yaml")
...
# Workspace trapezoid (matches verify_two_cubes.py).
YMIN, YMAX = 0.50, 0.80
HW_CLOSE, HW_FAR = 0.10, 0.18
CUBE_RADIUS = 0.03
MIN_CUBE_CUBE = 0.12
MIN_RESET_SHIFT = 0.10
MIN_CUBE_A_TO_PREV_CUBE_B = 0.20
...
def sample_pair(rng, max_outer=2000, max_inner=300,
    ...
    Matches verify_two_cubes.py: new poses must each be >= min_shift from
    their previous position, and the new cube_a must be >=
    min_cube_a_to_prev_cube_b from the still-standing cube_b (cube_a is
    placed BEFORE cube_b is moved during reset).
    """
...
def sample_reset_targets(rng, prev_cube_a, prev_cube_b):
    """Sample (cube_a_new, cube_b_new) with progressive constraint relaxation."""
    levels = [
        (MIN_RESET_SHIFT, MIN_CUBE_A_TO_PREV_CUBE_B, "strict"),
        (0.05,            0.16,                       "medium"),
        (0.03,            MIN_CUBE_CUBE,              "loose"),
        (0.00,            MIN_CUBE_CUBE,              "safety-only"),
    ]
    last_err = None
    for min_shift, min_ab, label in levels:
        try:
            pair = sample_pair(
                rng,
                prev_cube_a=prev_cube_a, prev_cube_b=prev_cube_b,
                min_shift=min_shift,
                min_cube_a_to_prev_cube_b=min_ab,
            )
            if label != "strict":
                print(f"  [sampler] fell back to '{label}' constraints "
                      f"(min_shift={min_shift}, min_cube_a_to_prev_cube_b={min_ab})")
            return pair
...
def main():
    ...
    ap.add_argument("--pick-z-table", type=float, default=0.150,
                    help="TCP z when picking a cube from the table")
    ap.add_argument("--pick-z-cube-on-cube", type=float, default=None,
                    help="TCP z when picking cube_a back off cube_b "
    ...
    ap.add_argument("--cube-a", type=str, default=None,
                    help="'x,y' override for cube_a initial position")
    ap.add_argument("--cube-b", type=str, default=None,
                    help="'x,y' override for cube_b initial position")
    ...
    params = {
        ...
        "pick_z_cube_on_cube": args.pick_z_cube_on_cube,
    ...
    if args.cube_a and args.cube_b:
        ...
        state = {"cube_a": list(cube_a), "cube_b": list(cube_b)}
    ...
    try:
        for ep in range(args.start_episode, args.start_episode + args.n_episodes):
            print(f"\n{'='*60}\nEPISODE {ep}\n{'='*60}")
            cube_a = tuple(state["cube_a"])
            cube_b = tuple(state["cube_b"])

            cube_a_new, cube_b_new = sample_reset_targets(rng, prev_cube_a=cube_a, prev_cube_b=cube_b)
            init_pose = sample_init()
            print(f"start: cube_a={cube_a}  cube_b={cube_b}")
            print(f"reset targets: cube_a_new={cube_a_new}  cube_b_new={cube_b_new}")
            print(f"init {tuple(round(v, 3) for v in init_pose)}")

            with open(POSITIONS_LOG, "a") as f:
                f.write(json.dumps({
                    "episode": ep,
                    "start": {"cube_a": list(cube_a), "cube_b": list(cube_b)},
                    "reset_targets": {"cube_a_new": list(cube_a_new), "cube_b_new": list(cube_b_new)},
                    "init_pose": list(init_pose),
                    "timestamp": time.time(),
                }) + "\n")

            # ----- RECORDED motion: pick cube_a, stack on cube_b -----
            print(f"--- {TASK_NAME} ---")
            go(*init_pose); time.sleep(0.5)

            def task_motion():
                pick("cube_a", *cube_a, pick_z=args.pick_z_table)
            ...
            meta = {
                **params,
                "episode": ep,
                "start": {"cube_a": list(cube_a), "cube_b": list(cube_b)},
                "end":   {"cube_a": list(cube_b), "cube_b": list(cube_b)},
                "init_pose": list(init_pose),
            }
            ...
            state = {"cube_a": list(cube_a_new), "cube_b": list(cube_b_new)}
            save_state(STATE_PATH, state)
