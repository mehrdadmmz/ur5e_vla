    ...
    async def _handle_command(self, websocket: WebSocket, data: Dict[str, Any]) -> None:
        ...
        try:
            if command_str == "pause":
                ...
                auto_start = params.get("auto_start", True)  # Default to auto-start
                    ...
                    if auto_start:
                        try:
                            ...
                            if adapter and adapter.is_initialized() and not adapter.is_running():
                                ...
                                if success:
                                    response["success"] = True
                                    response["message"] = f"Started execution with goal '{goal_name}'"
                                else:
                                    response["success"] = False
                                    response["message"] = f"Failed to start execution with goal '{goal_name}'"
                            elif adapter and adapter.is_running():
                                response["success"] = True
                                response["message"] = f"Goal '{goal_name}' will be applied on next cycle"
                            else:
                                response["success"] = False
                                response["message"] = "System not initialized. Connect first."
                        except Exception as e:
                            response["success"] = False
                            response["message"] = f"Error starting execution: {str(e)}"
                    else:
                        response["success"] = True
                        response["message"] = f"Goal '{goal_name}' set (not auto-started)"
                else:
                    response["success"] = False
