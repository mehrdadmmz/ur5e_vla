...
def get_input_elements(rtde_r, gripper, camera_images):
    ...
    input_elements = {
        ...
        # "prompt": "grasp blue pencilcase from drawer",
    ...
    if gripper_status_in == 1:
        change = np.array(tcp_pose) - np.array(past_pose)
        prediction = np.array([-0.10, 0.60, 0.20]) + change
    else:
        prediction = np.array([-0.10, 0.60, 0.20])
    past_pose = tcp_pose 
...
def main(args: Args) -> None:
    ...
    #sajjad - canssss
    rtde_c.moveL([0.0, 0.45, 0.50, 3.14, 0.0, 0.0], 0.5, 0.5)
    ...
    try:
        while True:
            ...
            final_gripper_cmd = action_plan[-1][6]
            ...
            if target_gripper_state != gripper_status:
                ...
                if target_gripper_state != 0:
                    ...
                    move_delta(rtde_c, rtde_r, [0.00, 0.04, -0.03])
                    time.sleep(1)
                # else:
