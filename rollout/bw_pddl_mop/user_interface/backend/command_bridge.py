...
class CommandBridge:
    ...
    def send_command(self, command: Command) -> None:
        ...
        if command.type == CommandType.PAUSE:
            ...
            with self._lock:
                self._pending_goal = command.params.get('goal_name')
                self._pending_goal_predicates = command.params.get('goal_predicates')
    ...
    def broadcast_state(self, observations: List[List],
                       ...
                       uncertain_ids: List[int] = None) -> None:
        """
        ...
        self.state_manager.update_state(predicates=logical_state)
